# ==============================================================================
# Supplement 2: Whole-Blood Fatty Acid Phenotype Calculator
# Developed for Whole-Blood Fatty Acid Profiling
#
# Instructions:
# 1. Ensure "Anonymous_Phenotype_Core.RData" is in the same working directory.
# 2. Input dataframe must contain the 29 required fatty acid variables.
#    An ID column is recommended but optional.
# ==============================================================================
...
predict_phenotype <- function(new_raw_df, core_file = "Anonymous_Phenotype_Core.RData") {
  ...
  if(any(is.na(new_mat))) {
    ...
    stop(paste0(
      ...
      "Missing values (NA) or non-numeric characters (e.g., 'ND', '<0.01') detected for the following profile IDs:\n",
      paste(bad_ids, collapse = ", "), "\n",
      "PCA projection requires complete feature matrices. Please correct the input records."
    ))
  }
  
  # 5. Batch-Adaptive Zero Imputation
  # Uses half the minimum observed nonzero value in the external dataset;
  # baseline half-minimum values are used as fallback when no positive value is available for a required fatty acid.
  clean_mat <- new_mat
  for(v in base_vars) {
    x <- clean_mat[, v]
    is_zero <- (!is.na(x) & x == 0)
  ...
  # 7. Fixed-Space PCA Projection and Phenotype Assignment (99% Threshold)
  ...
  p <- ggplot() +
    ...
    labs(x = "PC1", y = "PC2", title = "Phenotype Projection Map",
         subtitle = "New cohort projection into the fixed baseline phenotype space") +
    theme_minimal() +
    theme(
